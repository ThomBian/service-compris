...
describe('useCampaign', () => {
  ...
  it('advanceNight increments nightNumber and stores ledger', () => {
    const { result } = renderHook(() => useCampaign());
    const ledger = { cash: 500, netProfit: 200, rating: 4.0, coversSeated: 20 };
  ...
  it('fireCorkboard sets lossReason and stores ledger', () => {
    const { result } = renderHook(() => useCampaign());
    const ledger = { cash: 100, netProfit: -50, rating: 2.0, coversSeated: 5 };
  ...
  it('resetCampaign clears all state', () => {
    ...
    act(() => { result.current.advanceNight({ cash: 400, netProfit: 100, rating: 4.0, coversSeated: 15 }); });

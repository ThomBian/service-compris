...
export const FloorplanGrid: React.FC = () => {
  ...
  useEffect(() => {
    if (isSeating && selectedCells.length === partySize && partySize > 0) {
      // Small delay for visual feedback of the last selected cell
  ...
  return (
    <div className="flex flex-col gap-4 bg-stone-100 p-6 h-full overflow-y-auto custom-scrollbar">
      {/* Header */}
      ...
      <div 
        className={`
          grid gap-1 bg-stone-300 p-1 rounded-lg border-2 border-stone-400 aspect-square
        ...
        {grid.map((row, y) => (
          row.map((cell, x) => (
            <button
              ...
              className={`
                aspect-square rounded-sm transition-all duration-200 flex items-center justify-center

# Floorplan Grid Always-Visible Fix

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** The 4×4 floorplan grid must always be fully visible without scrolling, staying square and fitting within whatever height/width its column provides.

**Architecture:** Remove the `gridTemplateRows` style that conflicts with cell `aspect-square`, then use a `ResizeObserver` hook to measure the wrapper and cap grid width to `min(wrapperWidth, wrapperHeight)`.

**Tech Stack:** React 19, TypeScript, Tailwind CSS v4, standard browser `ResizeObserver` API.
...
The grid has both:
- `gridTemplateRows: repeat(4, minmax(0, 1fr))` → rows divide the grid container's height equally
- Cells have `aspect-square` → cell height = cell width = column width = grid_width / 4

When available height < available width, CSS caps grid height (via `max-h-full`). Rows shrink to `capped_height / 4`. But cells still want `height = grid_width / 4 > row_height` — they overflow their rows. Because the outer div has `overflow-hidden`, rows 2–4 are clipped. Only row 1 is visible.

## File Map

| File | Action | Responsibility |
|------|--------|---------------|
| `src/hooks/useContainerSize.ts` | Create | ResizeObserver hook returning `{ width, height }` of a ref'd element |
| `src/components/FloorplanGrid.tsx` | Modify | Remove patches, use hook to constrain grid to a square |

---

## Task 1: Create `useContainerSize` hook
...
```bash
cd /Users/tbianchini/workspace/service-compris && npm run lint
```
...
```bash
git add src/hooks/useContainerSize.ts
git commit -m "feat: add useContainerSize hook with ResizeObserver"
```

---

## Task 2: Rewrite FloorplanGrid layout

**Files:**
- Modify: `src/components/FloorplanGrid.tsx`

**What changes:**
- Remove the three nested wrapper divs added during patching (`relative`, `absolute inset-0`, centering div)
- Grid wrapper: plain `flex-1 min-h-0` div with a `wrapperRef`
- Grid element: `w-full` + `style={{ maxWidth: Math.min(width, height) }}` (from hook) — no `aspect-square` on the container, no `gridTemplateRows` (cells drive row height naturally)
- Outer div: `flex flex-col h-full overflow-hidden p-6 gap-4` — no scroll

**Why removing `gridTemplateRows` works:**
Each cell has `aspect-square` so cell height = column width = grid_width / 4. With no explicit row height, CSS grid auto-sizes rows to cell content → each row = grid_width / 4 → total grid height ≈ grid_width (naturally square). No `aspect-square` needed on the container.

- [ ] **Step 1: Replace FloorplanGrid.tsx**
...
Check at md breakpoint (768px+): grid is square, all 16 cells visible, no scroll, no clipping. Resize the browser window — grid should always fit. Enter seating mode — footer buttons appear below grid without clipping the grid.

- [ ] **Step 4: Commit**

```bash
git add src/components/FloorplanGrid.tsx
git commit -m "fix: floorplan grid always shows all 16 tiles without scrolling"
```

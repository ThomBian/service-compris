...
## 2. Session flow (landing → shift → corkboard)
The product is not only the live shift: it wraps each run in a light campaign shell.

1. **Landing** — Choose difficulty and start a shift (or return from the corkboard).
2. **Playing** — Real-time shift: queue, desk, floorplan, pause/speed, optional onboarding tour for new players.
3. **Corkboard** — After the shift ends: ledger-style summary, path-based campaign beats, or a **fired** outcome when morale or rules fail badly enough.

Campaign state (scores, night progression) bridges corkboard and the next shift so runs feel connected, not isolated arcade rounds.
...
## 4. Win / Loss Conditions (The Shift)
...
**Campaign-level loss:** Dropping morale or breaking rules too hard can end in a **fired** corkboard rather than a normal night wrap-up.

## 5. Localization (product framing)
The game ships **bilingual English and French**. Player-facing strings are centralized for translation; design and narrative specs should assume copy may appear in either language (and that activity logs store the wording from the locale active when the event was written unless we re-localize on read later).

---
## Changelog
- **v2.1.0:** Documented session/meta flow (landing, corkboard, campaign, fired), campaign bridge between runs, and EN/FR localization as part of the product concept.

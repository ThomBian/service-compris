# Floorplan Full-Page View Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the floorplan grid fill the full screen below the TopBar, reorder the header so the title is always first, and add drag-to-select for faster seat placement.

**Architecture:** Two files change. `ScenePanel.tsx` returns null in floorplan view so `BottomPanel`/`FloorplanGrid` gets all available height. `FloorplanGrid.tsx` gets a new header structure (title → party strip → rush row) and pointer-event drag handling on the grid wrapper and cells.

**Tech Stack:** React 19, TypeScript, Tailwind CSS 4, Framer Motion (motion/react), Lucide icons

---

## File Map

- Modify: `src/components/ScenePanel.tsx` — return null when `view === 'floorplan'`
- Modify: `src/components/floorplan/FloorplanGrid.tsx` — reorder header sections, add drag-to-select pointer events

---

### Task 1: ScenePanel — hide scene in floorplan view

**Files:**
- Modify: `src/components/ScenePanel.tsx`

The current component always renders a height div (70vh or 50vh). When `view === 'floorplan'` we return null so the flex sibling (`BottomPanel`) expands to fill all available height.

- [ ] **Step 1: Update ScenePanel to return null for floorplan view**

Replace the entire component body in `src/components/ScenePanel.tsx`:
...
Run `npm run dev`. Switch to floorplan view. The grid should now fill the full screen below the TopBar. Switch back to desk view — the desk scene should appear at normal height.
...
git commit -m "feat: hide scene panel in floorplan view for full-screen grid"
```

---

### Task 2: FloorplanGrid — reorder header sections

**Files:**
- Modify: `src/components/floorplan/FloorplanGrid.tsx`

Current order: combined header (title + seating text + buttons) → overtime row → grid.
New order: title-only header → party strip (seating mode) → rush row (overtime, not seating) → grid.

The party strip replaces the seating status text and moves the Confirm/Refuse buttons into its own row. The title has no subtitle.

- [ ] **Step 1: Rewrite FloorplanGrid layout**

Replace the full file `src/components/floorplan/FloorplanGrid.tsx`:
...
Run `npm run dev`. In seating mode the party strip should appear below the title with icons and buttons. In overtime (not seating) the rush row should appear below the title. The grid should stay in the same vertical position between states.
...
git commit -m "feat: reorder floorplan header — title, party strip, rush row, grid"
```

---

### Task 3: FloorplanGrid — drag-to-select

**Files:**
- Modify: `src/components/floorplan/FloorplanGrid.tsx`

Add two refs: `isDragging` (boolean) and `dragDirection` (`'select' | 'deselect'`). The grid wrapper gets `onPointerUp` and `onPointerLeave` to end drag. Cells get `onPointerDown` (replaces click as the selection trigger, sets direction) and `onPointerEnter` (continues drag). `touch-action: none` on the grid wrapper prevents scroll interference on touch devices. Toasts only fire on `pointerdown`, not during drag.

- [ ] **Step 1: Add drag refs and handlers to FloorplanGrid**

In `src/components/floorplan/FloorplanGrid.tsx`, after the existing `wrapperRef` and `useContainerSize` lines, add two refs and two new callbacks. Then wire them into the JSX.

Add after `const { width, height } = useContainerSize(wrapperRef);`:
...
- [ ] **Step 2: Wire handlers into the grid wrapper and cells**

In the grid wrapper `<div>` (the one with `id="floorplan-grid"`), add `onPointerUp`, `onPointerLeave`, and `style={{ touchAction: 'none' }}`:
...
On each cell `<button>`, replace `onClick` with `onPointerDown` and add `onPointerEnter`:
...
Remove the `onClick` prop entirely from the cell buttons.

- [ ] **Step 3: Verify manually**

Run `npm run dev`. In seating mode:
1. Single click on an empty cell — should select it (same as before).
2. Click and hold, drag across adjacent empty cells — each cell should select as the pointer enters it.
3. Click and hold on a selected cell, drag over other selected cells — should deselect them.
4. Auto-confirm should still fire 300ms after reaching party size, whether via click or drag.
5. On a touch device (or DevTools touch simulation) — drag should select cells without scrolling the page.
...
```bash
git add src/components/floorplan/FloorplanGrid.tsx
git commit -m "feat: drag-to-select cells on floorplan grid"
```
